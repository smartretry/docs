...
<Note>
  SmartRetry has access to card details and transaction data as part of the retry and optimization process. Card numbers are not stored, and all sensitive data is encrypted in transit and at rest. See [Security and compliance](/security) for details.
</Note>

## Token-based payments

For a tokenized payment, send the 88-character SmartRetry `token` at the top level of the sale or pre-authorization request. A normal successful payment response does not return a token. Retrieve a stored payment-instrument token from the [transaction status response](/api-reference/payments/status) when available.

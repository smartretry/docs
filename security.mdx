...
SmartRetry does not store card numbers. For repeat billing, send a stored payment token instead of raw card details - see [Payment methods](/integration/payment-methods).
...
API keys authenticate server-to-server requests only. Never expose a key client-side, keep keys out of source control, and use separate keys per environment. See [Authentication](/authentication) for handling and rotation.
...
Your webhook endpoint is a public URL - anyone can post to it. Verify the signature on every incoming payload before acting on it, and reject anything that fails. Treat delivery as at-least-once and key your processing on the event ID. See [Webhooks](/integration/webhooks) for the signature scheme and verification examples.
